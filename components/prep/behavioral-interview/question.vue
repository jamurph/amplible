<script setup>
            ...
            if(!gettingCritique.value){
                ...
                try {
                    ...
                    if(result.error.value){
                        ...
                        emit('addAnswer', props.question.id, answer, order, critique) 
                    }
                }
                catch (er){
    ...
    async function advise(){
        if(!gettingAdvice.value){
            ...
            try {
                const result = await useFetch(() => `/api/behavioral-interview/question/advise/${props.question.id}`, {
                    method: 'GET',
                    headers: useRequestHeaders(['cookie']) //necessary for serverSupabase
                })
                if(result.error.value){
                    ...
                    adviceError.value = ''
                    //emit event to parent, so that it can update the qaTree.
                    emit('setAdvice', props.question.id, result.data.value.text)
                }
            }
            catch (er){
    ...
    process How - DONE
        ...
        hide how link - DONE
    process submit
        validation & Error Processing - DONE
        finish post response - DONE
        show answers + critiques in normal load flow (should match up with submitting flow) - DONE
        when submitting, change to show answer while not in textarea - DONE


        try again button - DONE
        - click sets ref, shows new textarea and submit btn. - DONE
        - process try again submit w/ same stuff as other submit. SHOULD just work. - DONE
        
    next button (not in question.vue)
        - instead of immediately show next question after answer. show button to show next.
    make sure the qaTree get only returns the text for each advice, answer, and critique.
    transitions?
    ...
    <div class="mb-16">
        <div class="bg-dark-light10 md:py-6 md:px-10 p-4 rounded-lg text-lg shadow-lg">
            ...
            <div v-if="!props.question.skipped">
                ...
                <div v-if="props.question.behavioral_interview_question_advice.length > 0" class="bg-dark-light20 shadow-sm mt-3 px-4 py-6  border-l-4 rounded-lg border-secondary text-base">
                    <div class="font-bold text-lg mb-3">Answer Advice:</div>
                    {{ props.question.behavioral_interview_question_advice[0].text }}
                </div>
                <LinkPrimary v-if="props.question.behavioral_interview_question_advice.length === 0 && props.question.behavioral_interview_question_answer.length === 0 && !gettingCritique" class="inline-block mt-3 text-sm" @click="advise">{{gettingAdvice ? 'Loading...' : 'Get Answer Advice'}}</LinkPrimary>
                <small v-if="adviceError" class="text-sm mt-1 text-red-400 block">Error getting AI advice. Please try again or refresh.</small>
                <div v-for="answer in props.question.behavioral_interview_question_answer" :key="answer.id">
                    <div class="bg-dark-light20 shadow-sm mt-3 px-4 py-6  border-r-4 rounded-lg border-primary text-base">
                        <div class="font-bold text-lg mb-3">Answer:</div>
                        {{ answer.text }}
                    </div>
                    <div class="bg-dark-light20 shadow-sm mt-3 px-4 py-6  border-l-4 rounded-lg border-secondary text-base">
                ...
                <textarea v-if="props.question.behavioral_interview_question_answer.length === 0 && !gettingCritique"
                    class="rounded mt-6 p-2 w-full text-dark bg-light border-2"
                    name="answer"
                    ...
                    rows="5"
                ></textarea>
                <div v-if="gettingCritique && !tryingAgain" class="bg-dark-light20 shadow-sm mt-3 px-4 py-6  border-r-4 rounded-lg border-primary text-base">
                    <div class="font-bold text-lg mb-3">Answer:</div>
                    {{ formData.answer }}
                </div>
                <div class="text-right my-4" v-if="!gettingCritique && props.question.behavioral_interview_question_answer.length === 0">
                    <LinkDefault class="mt-3 inline-block text-sm mr-8" @click="() => setSkipped(true)">{{settingSkipped ? 'Loading...' : 'Skip'}}</LinkDefault>
                    <ButtonPrimary class="inline-block" @click="submitAnswer">Submit</ButtonPrimary>
                </div>
                <div class="text-right my-4" v-if="props.question.behavioral_interview_question_answer.length > 0 && !tryingAgain && !gettingCritique">
                    <ButtonDefault class="inline-block" @click="tryingAgain = true">Try Another Answer</ButtonDefault>
                </div>
                <div v-if="tryingAgain">
                    <textarea v-if="!gettingCritique"
                    class="rounded mt-6 p-2 w-full text-dark bg-light border-2"
                    name="answer"
                    placeholder="Give it your best shot!"
                        v-model="formData.answer"
                        ...
                        rows="5"
                        ></textarea>
                        <div v-if="gettingCritique" class="bg-dark-light20 shadow-sm mt-3 px-4 py-6  border-r-4 rounded-lg border-primary text-base">
                            <div class="font-bold text-lg mb-3">Answer:</div>
                            {{ formData.answer }}
                        </div>
                ...
                <small v-if="v$.answer.$error" class="text-sm mt-1 text-red-400 block">{{ v$.answer.$errors[0].$message }}</small>
                <small v-if="critiqueError" class="text-sm mt-1 text-red-400 block">Error connecting to the server. Please refresh the page or try again.</small>
        ...
        <div class="pt-1 text-sm text-light-dark20">
            <div class="sm:pl-16 pt-1 text-sm text-light-dark20 inline-block mb-1 sm:mb-0" v-if="props.question.skipped">
                You skipped this question. <LinkDefault class="pl-4" @click="() => setSkipped(false)">{{settingSkipped ? 'Loading...' : 'Unskip'}}</LinkDefault>
                <small v-if="skippingError" class="text-sm mt-1 text-red-400 block text-center">Error connecting to the server. Please refresh this page.</small>
            </div>
